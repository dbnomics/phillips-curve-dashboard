...
def create_phillips_curve(df, country):
    ...
    # Créer le graphique
    fig = px.scatter(df, x='unemployment_rate', y='inflation_rate', title=f'Phillips Curve for {country}',
                     labels={'unemployment_rate': 'Unemployment Rate (%)', 'inflation_rate': ' Inflation Rate (%)'},
                     hover_data={'original_period': True},
                     color_discrete_sequence=['darkblue'],
                     width=1200, height=500)

    fig.add_trace(
        go.Scatter(x=trend_df['unemployment_rate'], y=trend_df['inflation_rate'], mode='lines', name='Tendency',
...
df_fr = pd.merge(df_employ_fr, df_infla_fr, on='original_period').drop(columns=['WEO Country'])
df_us = pd.merge(df_employ_us, df_infla_us, on='original_period').drop(columns=['WEO Country'])
df_ar = pd.merge(df_employ_ar, df_infla_ar, on='original_period').drop(columns=['WEO Country'])
df_jp = pd.merge(df_employ_jp, df_infla_jp, on='original_period').drop(columns=['WEO Country'])
...
countries = st.multiselect('**Select Country**', ['France', 'United States', 'Argentina','Japan'],
                           default=['France'])
...
st.write("Unemployment rate : [*link*](https://db.nomics.world/ILO/UNE_DEAP_SEX_AGE_EDU_RT?tab=list)")
